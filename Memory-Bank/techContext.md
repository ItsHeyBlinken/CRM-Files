...
PRO_STRIPE_PRICE_ID=price_...          # Standard Pro ($29/mo) — used after founding cap
...
**Planned (Path B+):** Optional **per-invoice** `stripe_payment_link` on `invoices` table — client pay opens invoice-specific URL; fallback to vendor default. Still no Connect. See `activeContext.md`.
